...
class Node:
    def __init__(self, model, game, config, state, reward, parent=None, action_taken=None, prior=None, visit_count=0):
        self.model = model
        self.game = game
        self.config = config
        self.state = state
    ...
    def get_ucb(self, child):
        ...
        if child.visit_count == 0:
            ...
            q_value = self.game.get_opponent_value(child.total_value / child.visit_count)
    ...
    def expand(self, policy):
        ...
        child_state, reward = self.model.dynamics(
            torch.tensor(child_state, dtype=torch.float32, device=self.config.device), actions)
...
class MCTS:
    ...
    def search(self, state, valid_moves):
        ...
        return action_probs, root.total_value / root.visit_count
