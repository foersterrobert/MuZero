...
# %%
class CarRacing:
    def __init__(self, sequence_lenth=3, skip_frames=2, clip_reward=False, time_out_tolerance=100, time_out=25, render=False):
...
class Trainer:
    ...
    def self_play(self, game_idx):
        ...
        while True:
            ...
            game_memory.append((observation, action, action_probs, root.total_value / root.visit_count, reward, is_terminal))
            observation, reward, is_terminal = self.game.step(action)

            if is_terminal:
                return_memory = []
                for hist_state, hist_action, hist_action_probs, hist_root_value, hist_reward, hist_terminal in game_memory:
...
# testGame = CarRacing(render=True)

# muZero = MuZero(testGame, device).to(device)
...
# for i in range(1):
#     observation, reward, is_terminal = testGame.get_initial_state()
...
#             # print(action)
#             observation, reward, terminated = testGame.step(action)
