...
class CarRacingParallel:
    ...
    def get_initial_state(self):
        ...
        action, reward = np.zeros(self.num_parallel_games), [0] * self.num_parallel_games
    ...
    def step(self, action):
        prev_terminal = self.terminal_list.copy()
        action_plane = prev_terminal.copy()
        action_plane[action_plane == 0] = action
        reward = 0
        ...
        observation = self.get_encoded_observation(observation[prev_terminal == 0], action_plane)
...
class Trainer:
    ...
    def self_play(self, game_idx_group, self_play_bar):
        ...
        while len(self_play_games) > 0:
            ...
            observation, reward, is_terminal = self.game.step(np.array(action_list))
            ...
            for i, self_play_game in enumerate(self_play_games):
                if is_terminal[terminal_idx[i]]:
                    ...
                    self.game.obs_stack = np.delete(self.game.obs_stack, i, axis=0)
