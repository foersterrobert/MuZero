...
# %%
class CarRacingParallel:
    def __init__(self, num_parallel_games, sequence_lenth=3, skip_frames=3, clip_reward=False, time_out_tolerance=100, time_out=25, render=False):
        self.env = gym.vector.make('CarRacing-v2', num_envs=num_parallel_games, continuous=False, domain_randomize=False, render_mode='human' if render else 'rgb_array')
        ...
        self.counter = 0
        self.negative_reward_counter = np.zeros(self.num_parallel_games)
        self.terminal_list = np.zeros(self.num_parallel_games)
    ...
    def get_initial_state(self):
        ...
        action, reward, self.terminal_list = np.zeros(self.num_parallel_games), [0] * self.num_parallel_games, np.zeros(self.num_parallel_games)
        observation = self.get_encoded_observation(observation, action)
        self.obs_stack = observation.repeat(self.sequence_lenth, axis=1)
        return self.obs_stack, reward, self.terminal_list
    ...
    def step(self, action):
        ...
        for _ in range(self.skip_frames + 1):
            ...
            reward += r
            if any(is_terminal):
                self.terminal_list[is_terminal == True] = 1
        observation = self.get_encoded_observation(observation, action)
        self.obs_stack = np.concatenate((self.obs_stack[:, 1:], observation), axis=1)
        if self.clip_reward:
            reward = np.clip(reward, -float('inf'), 1)
        if self.counter > self.time_out_tolerance:
            self.negative_reward_counter[reward < 0] += 1
            self.terminal_list[self.negative_reward_counter > self.time_out] = 1
        else:
            self.counter += 1
        return self.obs_stack, reward, self.terminal_list
    ...
    def get_encoded_observation(self, observation, action):
        ...
        observation = Image.fromarray(observation).convert('L').crop((0, 0, 96*self.num_parallel_games, 84))
        observation = np.array(observation).astype(np.float32)

        action = action.astype(np.float32) * (255 / self.action_size)
        action_plane = action.reshape(1, self.num_parallel_games).repeat(12, axis=0).repeat(96, axis=1)

        observation = np.concatenate((observation, action_plane), axis=0)
        observation /= 255
        observation = np.stack(
            [observation[:, 96*i:96*(i+1)] for i in range(self.num_parallel_games)]
        ).reshape(self.num_parallel_games, 1, 96, 96)
        return observation
...
class Trainer:
    ...
    def self_play(self, game_idx_group, self_play_bar):
        self_play_games = [SelfPlayGame(game_idx_group * self.args['group_size'] + i) for i in range(self.args['group_size'])]
        ...
        while len(self_play_games) > 0:
            ...
            hidden_state = torch.tensor(observation[self.game.terminal_list == 0], dtype=torch.float32, device=self.device)
            ...
            for i, self_play_game in enumerate(self_play_games):
                self_play_game.canonical_observation_root = observation[i].copy()
                self_play_game.root = Node(
                    hidden_state[i], # cannonical if 2 players
                    0, # reward should not be needed
            ...
            action_list = {i:0 for i in range(self.args['group_size'])}
            terminal_idx = [i for i in range(self.game.terminal_list.shape[0]) if self.game.terminal_list[i] == 0]
            ...
            for i, self_play_game in enumerate(self_play_games):
                ...
                self_play_game.game_memory.append((
                    self_play_game.canonical_observation_root, action, action_probs, 
                    self_play_game.root.total_value / self_play_game.root.visit_count, 
                    reward[terminal_idx[i]], False
                ))
                
                action_list[terminal_idx[i]] = action
            
            observation, reward, is_terminal = self.game.step(np.array(list(action_list.values())))

            for i, self_play_game in enumerate(self_play_games):
                if is_terminal[terminal_idx[i]]:
                    return_memory = []
                    ...
                    if not self.args['K'] > 0:
                        self.replayBuffer.memory.append((
                            observation[terminal_idx[i]].copy(),
                            None,
...
game = CarRacingParallel(num_parallel_games=args['group_size'])
...
# game = CarRacingParallel(3, render=False)

# muZero = MuZero(game, device).to(device)
# muZero.load_state_dict(torch.load("Weights/CarRacing/model.pt", map_location=device))
# muZero.eval()

# mcts = MCTS(muZero, game, args)
...
#         # print(action)
#         observation, reward, is_terminal = game.step(np.full((3,), action))
